...
*/
		function lastDigit(int1, int2) {
			...
			if (int1String[int1String.length - 1] === int2String[int2String.length - 1]) {
	  			return true;
			}
	  	return false;
		}
...
*/
		function oddOnly(ints) {
			oddArray = [];
			for (let i = 0; i < ints.length; i++) {
				if (ints[i] % 2 === 1) {
					oddArray.push(ints[i]);
				} 
			}
			return oddArray;
		}
...
*/
		function cigarParty(int, boolean) {
			if ((int >= 40) && (boolean === true)) {
				return true;
			}
			else if ((int >= 40) && (int <= 60) && (boolean === false)) {
				return true;
			}
			return false;
		}
...
10. **filterBigNumbers** Write a function that filters numbers greater than or equal to 100.
	...
	filterBigNumbers([]) → []
*/
	function filterBigNumbers(ints) {
		for (let i = 0; i < ints.length; i++) {
			if (ints[i] < 100) {
				ints.splice(i, 1);
				i--;
...
*/
	function filterMultiplesOfX(ints, x) {
		for (let i = 0; i < ints.length; i++) {
			if (! (ints[i] % x === 0)) {
				ints.splice(i, 1);
				i--;
