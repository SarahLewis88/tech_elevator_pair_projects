 ...
 */
 ...
 function isAdmitted (gpa, satScore, recommendation) {
     if (gpa > 4 || satScore > 1300 || (gpa > 3 && recommendation == true) || (satScore > 120 && recommendation == true)) {
         return true;
     }
     return false;
 }
 ...
 * 
 * @param {Object[]} objArray object array
 * @returns {String[]} string array
 * 
 */
function getFullAddressesOfProperties(objArray) {
    let address = objArray.map((array) => {
 ...
 * 
 */
function findLargest(stringArray) {
    let largestNum = [0];
    stringArray.forEach(numArray => {
        if (largestNum < numArray) {
            largestNum = numArray;
            }
    });
    return largestNum;
