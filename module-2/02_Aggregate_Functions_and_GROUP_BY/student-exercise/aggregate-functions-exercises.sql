...
FROM country
WHERE gnp IS NOT NULL AND population IS NOT NULL AND population != 0
...
SELECT name, (gnp - gnpold) AS gnp_diff, ABS(gnp - gnpold) AS gnp_abs_diff
FROM country
...
SELECT countrycode, MAX(population) AS biggest_city
FROM city
GROUP BY countrycode
...
SELECT continent, MIN(lifeexpectancy) AS min_life, MAX(lifeexpectancy) AS max_life, AVG(lifeexpectancy) AS avg_life
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
